...
//        box1.set(new String("hello"));
        ...
        //정수 자료형으로 설정 후 6을 대입
//        Box<int> box2 = new Box<int>();  //<int> 사용 불가.
...
//        Box<double> box4 = new Box<double>();   // double 기초형으로해서 오류
        Box<Double> box4 = new Box<Double>();
//        box4.set(Double.valueOf(3.6));  //불필요한 박싱
        box4.set(3.6);
//        double data = box4.get().doubleValue();  //불필요한 언박싱
        double data = box4.get();
        System.out.println(data);

        Box<Integer> box5 = new Box<Integer>();
        box5.set(5);
//        box5.set(5.7);  // 오류함 box5는 Integer 타입이므로
